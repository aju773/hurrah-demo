...
describe("page heading", () => {
  it("names each page of the journey in a heading", () => {
    ...
    expect(screen.getByRole("heading", { level: 2, name: "Choose your options" })).toBeInTheDocument();
    rerender(withIntl(<PageHeading page="artwork" />));
    expect(screen.getByRole("heading", { level: 2, name: "Add your artwork" })).toBeInTheDocument();
    rerender(withIntl(<PageHeading page="approve" />));
    expect(screen.getByRole("heading", { level: 2, name: "Approve your order" })).toBeInTheDocument();
  });
  ...
  it("moves focus to the heading and announces the page when the page changes", () => {
    const { rerender } = render(withIntl(<PageHeading page="options" />));
    screen.getByRole("heading", { level: 2 }).blur();
    rerender(withIntl(<PageHeading page="artwork" />));
    expect(screen.getByRole("heading", { level: 2, name: "Add your artwork" })).toHaveFocus();
    expect(screen.getByRole("status")).toHaveTextContent("Page 2 of 3: Add your artwork");
    rerender(withIntl(<PageHeading page="approve" />));
    expect(screen.getByRole("heading", { level: 2, name: "Approve your order" })).toHaveFocus();
