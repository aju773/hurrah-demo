...
import { chooseBack, chooseFront, evaluateChoice, pageSizeLabel } from "@/lib/pagePicker";
 ...
 *
 * `onConfirm({front, back})` gets 1-based page numbers (back may be null). `busy`
 * shows the pages are being checked; `error` is a {code, message} from a refused
 * choice. Thumbnails are lazy <img>s: the server renders each on first request.
 */
export default function PagePicker({ source, orderedSize, busy = false, error = null, onConfirm, onCancel }) {
  const t = useTranslations("PagePicker");
  const tErrors = useTranslations("ArtworkErrors");
  const [choice, setChoice] = useState({ front: null, back: null });
  ...
  const result = evaluateChoice({ pages: source.pages, front: choice.front, back: choice.back, orderedSize });
    ...
    <div className="fixed inset-0 z-50 flex items-stretch sm:items-center justify-center bg-black/40 sm:p-[16px]" onKeyDown={onKeyDown}>
      ...
      >
        <div className="flex flex-col gap-[2px]">
          ...
          <p className="text-[#5d3f3e] text-[13px]">{t("instructions")}</p>
        </div>
        ...
        <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-[12px] overflow-y-auto min-h-0 list-none p-0 m-0" aria-label={t("pagesLabel")}>
          {source.pages.map((page) => {
            const isFront = choice.front === page.number;
            const isBack = choice.back === page.number;
            return (
              <li
                key={page.number}
              ...
              >
                <div className="bg-[#f0f3ff] rounded-[6px] flex items-center justify-center aspect-[3/4] overflow-hidden" dir="ltr">
                ...
                <div className="flex gap-[6px]">
                  <button
                    ...
                    disabled={busy}
                    onClick={() => setChoice((c) => chooseFront(c, page.number))}
                  ...
                  <button
                    ...
                    disabled={busy}
                    onClick={() => setChoice((c) => chooseBack(c, page.number))}
        ...
        <div className="flex flex-col gap-[6px]" aria-live="polite">
          <p className="text-[13px] text-[#151c27]" data-testid="picker-summary">
            {t("summary", { front: choice.front ?? t("none"), back: choice.back ?? t("noBack") })}
          </p>
        ...
        <div className="flex flex-wrap gap-[8px] justify-end">
          ...
          <button
            ...
            onClick={() => onConfirm({ front: choice.front, back: choice.back })}
