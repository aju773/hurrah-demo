...
export default function SiteHeader({ commerceEnabled = false }) {
  ...
  return (
    <>
      {/* Below lg the absolutely-positioned desktop header would run off the screen. */}
      <div className="lg:hidden flex flex-col gap-[12px] bg-white px-[16px] py-[12px]">
        <div className="flex items-center justify-between gap-[12px]">
          <img src="/assets/10b7e.png" className="h-[44px] w-[144px] object-cover" alt="Hurrah logo" />
          {commerceEnabled && (
            <a href="#" className="tap flex items-center justify-center rounded-[7px] bg-[#f0f3ff] px-[14px] py-[10px]">
              <img src="/assets/e897f.svg" className="w-[26px] h-[24px] -scale-x-100" alt={t("cartAlt")} />
            </a>
          )}
        </div>
        <div className="flex items-center h-[48px] w-full p-[2px] rounded-[8px] bg-[#f0f3ff]">
          <p className="flex-1 min-w-0 px-[12px] text-[#575c64] text-[14px] truncate">{t("searchPlaceholder")}</p>
          <div className="bg-[#ffc72c] flex h-[44px] items-center justify-center px-[16px] rounded-[8px] shrink-0">
            <img src="/assets/9031c.svg" className="w-[15px] h-[15px]" alt="search" />
          </div>
        </div>
      </div>

      <div className="hidden lg:block relative w-full" style={{ height: "151px" }}>
        <div
